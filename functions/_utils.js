...
const FARM_IDS = new Set(["farm-01", "farm-02", "farm-03", "farm-04", "farm-05"]);
...
export function getFarmKeys(env) {
  try {
    const parsed = JSON.parse(env.FARM_ADMIN_KEYS_JSON || "{}");
...
export function isAllowedFarmId(farmId) {
  return FARM_IDS.has(String(farmId || ""));
}
...
export async function requireFarm(context) {
  ...
  if (!session?.farmId) {
    return { response: errorJson("ログインが必要です。", 401) };
  }
...
export async function ensureFarmerRow(env, farmId) {
  ...
  await env.DB.prepare(
    "INSERT OR IGNORE INTO farmers (id, name, description, is_public) VALUES (?, ?, ?, 1)"
  )
    .bind(farmId, defaultFarmName(farmId), "運用実証用の農園アカウントです。")
    .run();
}

export function defaultFarmName(farmId) {
  const n = String(farmId || "").replace("farm-", "");
  return `〇〇農園 ${Number(n) || 1}`;
}
...
export function createRecordId(farmerId, productName, date) {
  ...
  return productSlug
    ? `${sanitizeSegment(farmerId, "farm")}-${productSlug}-${date}`
    : `${sanitizeSegment(farmerId, "farm")}-${date}`;
}
